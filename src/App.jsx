 ...
 * Calendar fix: proper two-part cell structure (header / body)
 */
...
(() => {
  ...
  s.textContent = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-secondary:hover  { background:#F9FAFB; border-color:#D1D5DB; box-shadow:0 1px 4px rgba(0,0,0,.05); }
    ...
    /* ── Status tags ── */
    .tag { display:inline-flex; align-items:center; gap:5px; padding:3px 9px; border-radius:20px; font-size:11px; font-weight:600; border:1px solid transparent; white-space:nowrap; }
    ...
    /* ── Kanban ── */
    .k-col { flex-shrink:0; width:268px; background:#F9FAFB; border:1px solid #E5E7EB; border-radius:12px; display:flex; flex-direction:column; }
    .k-card { background:#fff; border:1px solid #E5E7EB; border-radius:10px; padding:13px; margin:0 8px 8px; cursor:pointer; transition:border-color .15s, box-shadow .15s, transform .15s; }
    ...
    .cmd-wrap { position:fixed; inset:0; z-index:200; display:flex; align-items:flex-start; justify-content:center; padding-top:14vh; background:rgba(0,0,0,.22); backdrop-filter:blur(4px); }

    /* ═══════════════════════════════════════════════════════════
       CALENDAR CELL — two-part fixed structure
       ═══════════════════════════════════════════════════════════
       Each cell is a flex column with:
         .cal-cell-head  — fixed height, holds the day number
         .cal-cell-body  — remaining space, scrollable event list

       This guarantees the date number is ALWAYS at the top and
       event chips ALWAYS start below it, with zero overlap.
    ══════════════════════════════════════════════════════════════ */

    .cal-grid {
      display: grid;
      grid-template-columns: repeat(7, 1fr);
      /* Each row is exactly 110px tall — enforced on every cell */
    }

    .cal-cell {
      /* Fixed height — every cell in a row is the same */
      height: 110px;
      /* Two-part flex column: header on top, body fills rest */
      display: flex;
      flex-direction: column;
      border-right: 1px solid #F3F4F6;
      border-bottom: 1px solid #F3F4F6;
      overflow: hidden;           /* nothing escapes the cell bounds */
      cursor: pointer;
    ...
    .cal-cell.blank    { background: #FAFAFA; cursor: default; }

    /* Part 1 — date number header: fixed 28px, never shrinks */
    .cal-cell-head {
    ...
    .cal-day-num.today    { background: #FDE68A; color: #92400E; }
    .cal-day-num.selected { background: #C7D2FE; color: #4338CA; }
    .cal-day-num.sel-today{ background: #6366F1; color: #fff;    }
    ...
    /* Part 2 — event body: fills remaining height, clips overflow */
    .cal-cell-body {
      flex: 1;                    /* takes all space left after header */
      overflow: hidden;           /* clip — no scrollbar on cell */
      display: flex;
      flex-direction: column;
      gap: 2px;
      padding: 0 5px 4px;
    ...
    /* Individual event chip inside a cell */
    .cal-event {
      display: flex;
      align-items: center;
      gap: 4px;
      ...
      /* Hard truncate — chip never wraps or resizes */
      min-width: 0;
      flex-shrink: 0;
    }
    .cal-event:hover {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchAll(true);
    const ch = supabase.channel("dh-cal-v2")
  ...
  async function addPost(post) {
    if (!supabase) return;
    const payload = { ...post, video_link:normUrl(post.video_link||""), status:post.status||"Draft", updated_at:new Date().toISOString() };
    // only include priority if caller passed it (column may not exist in older schemas)
  ...
  async function updatePost(id, fields) {
    ...
    const payload = { ...fields, video_link:normUrl(fields.video_link||""), updated_at:new Date().toISOString() };
    if (!payload.priority) delete payload.priority;
...
function Sidebar() {
  ...
  return (
    <aside style={{width:sidebar?220:48,flexShrink:0,background:"#fff",borderRight:"1px solid #E5E7EB",display:"flex",flexDirection:"column",transition:"width 0.2s cubic-bezier(.32,.72,0,1)",overflow:"hidden",zIndex:20}}>
      {/* Logo */}
      <div style={{padding:"14px 8px 10px",borderBottom:"1px solid #F3F4F6",flexShrink:0}}>
      ...
      <div style={{padding:"8px 6px",flex:1,overflowY:"auto"}}>
        {sidebar && <p style={{fontSize:10,fontWeight:600,color:"#9CA3AF",textTransform:"uppercase",letterSpacing:"0.1em",padding:"4px 8px 5px"}}>Navigation</p>}
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.id} onClick={()=>setView(item.id)}
              ...
              title={item.label}
              style={{marginBottom:1,justifyContent:sidebar?"flex-start":"center",padding:sidebar?"7px 10px":"7px"}}>
        ...
        {sidebar && (
          <>
            <div style={{height:1,background:"#F3F4F6",margin:"10px 2px"}}/>
...
function TopBar() {
  ...
  return (
    <div style={{height:52,borderBottom:"1px solid #E5E7EB",background:"#fff",display:"flex",alignItems:"center",padding:"0 20px",gap:10,flexShrink:0,boxShadow:"0 1px 3px rgba(0,0,0,.03)"}}>
      ...
      {showFilters && (
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {[
            {val:filterStatus,   set:setFS,   opts:["all",...STATUSES],  label:"Status"},
            {val:filterPlatform, set:setFP,   opts:["all",...PLATFORMS], label:"Platform"},
            {val:sortBy,         set:setSort, opts:["date","title","status"], label:"Sort"},
          ].map((f,i)=>(
      ...
      <button onClick={()=>setCmdOpen(true)}
        style={{display:"flex",alignItems:"center",gap:8,background:"#F9FAFB",border:"1px solid #E5E7EB",borderRadius:8,padding:"5px 12px",cursor:"pointer",color:"#9CA3AF",fontSize:12,minWidth:160,fontFamily:"'Inter',sans-serif",transition:"border-color 0.15s"}}
  ...
  return (
    <div style={{maxWidth:960,margin:"0 auto"}}>
      <div style={{marginBottom:28}}>
        <h1 style={{fontSize:26,fontWeight:700,color:"#111827",marginBottom:4,letterSpacing:"-0.3px"}}>
          Deeper Healing Content Calendar
        </h1>
...
// ─── Calendar ─────────────────────────────────────────────────────────────────
// ROOT CAUSE OF THE ALIGNMENT BUG (now fixed):
//
//   Before: each cell used `minHeight` + a single flex div where the date
//   number and event chips were siblings in the SAME flow. This caused the
//   date number to push down when events were added, and different cells in
//   the same row ended up at different heights — making the whole grid
//   look misaligned.
//
//   Fix: every cell is now EXACTLY 110px tall via the `.cal-cell` class
//   and split into two hard-separated containers:
//
//     .cal-cell-head  (fixed 28px) — date number only, never moves
//     .cal-cell-body  (flex:1)     — event chips only, clipped at cell edge
//
//   The date number is structurally isolated from events so it can never
//   be displaced. Long titles truncate inside the chip; overflow chips are
//   replaced with a "+N" label. Row heights are uniform by construction.
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16}}>
      ...
      <div className="card" style={{overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        {/* Header */}
        <div style={{padding:"16px 20px 14px",borderBottom:"1px solid #F3F4F6",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        {/* ── Calendar grid ──
            Uses the .cal-grid class which sets grid-template-columns:repeat(7,1fr).
            Every cell is .cal-cell with a fixed 110px height and flex-column
            layout. Date number lives in .cal-cell-head (fixed 28px).
            Events live in .cal-cell-body (flex:1, overflow:hidden).
        ── */}
        <div className="cal-grid" style={{flex:1}}>
          {grid.map((cell,i) => {
            ...
            // Show at most 2 chips; remaining → "+N"
            const visible  = cell.posts.slice(0, 2);
            const overflow = cell.posts.length - visible.length;

            let cellClass = "cal-cell";
            if (isSel)   cellClass += " selected";
            if (isToday && !isSel) cellClass += " today";

            let numClass  = "cal-day-num";
            if (isBoth)  numClass += " sel-today";
            else if (isSel)  numClass += " selected";
            else if (isToday) numClass += " today";
            ...
            return (
              ...
              >
                {/* ── Part 1: date number header — FIXED, never moves ── */}
                <div className="cal-cell-head">
                  <span className={numClass}>{cell.day}</span>
                  {cell.posts.length > 0 && (
                    <span className="cal-post-count">{cell.posts.length}</span>
                  )}
                </div>

                {/* ── Part 2: event body — fills remaining space ── */}
                <div className="cal-cell-body">
                  {visible.map(p => {
                    ...
                    return (
                      ...
                      >
                        <span className={`cal-event-dot ${sm.dot}`}/>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div className="card" style={{overflow:"hidden"}}>
          <div style={{padding:"14px 16px 12px",borderBottom:"1px solid #F3F4F6"}}>
            <p style={{fontSize:10,color:"#9CA3AF",textTransform:"uppercase",letterSpacing:"0.08em",fontWeight:600,marginBottom:2}}>Selected</p>
        ...
        <div className="card" style={{flex:1,overflow:"hidden"}}>
          <div style={{padding:"14px 16px 12px",borderBottom:"1px solid #F3F4F6",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              <p style={{fontSize:10,color:"#9CA3AF",textTransform:"uppercase",letterSpacing:"0.08em",fontWeight:600,marginBottom:2}}>Queue</p>
            ...
            {upcoming.slice(0,8).map(p=>{
              ...
              return (
                <div key={p.id} onClick={()=>setEdit(p)}
                  style={{padding:"10px 16px",borderBottom:"1px solid #F9FAFB",display:"flex",alignItems:"center",gap:8,cursor:"pointer",transition:"background 0.1s"}}
                  className="row-hover">
                  <span className={sm.dot} style={{width:7,height:7,borderRadius:"50%",flexShrink:0}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontSize:13,fontWeight:500,color:"#111827"}} className="truncate">{p.title}</p>
...
// Expandable post row inside the "Selected day" panel
function CalPostRow({ post }) {
  ...
  return (
    <div style={{borderBottom:"1px solid #F9FAFB"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 6px",borderRadius:7,transition:"background 0.1s",cursor:"pointer"}} className="row-hover">
        ...
        <button onClick={()=>setOpen(o=>!o)} className="btn-ghost" style={{padding:4}}>
          <ChevronDown style={{width:11,height:11,transform:open?"rotate(180deg)":"none",transition:"transform 0.15s"}}/>
        </button>
      </div>
      {open && (
        <div className="fade-in" style={{padding:"6px 10px 10px",background:"#F9FAFB",borderRadius:7,marginBottom:4}}>
        ...
        {STATUSES.map(status=>{
          ...
          return (
            <div key={status} className="k-col">
              <div style={{padding:"12px 14px 10px",borderBottom:"1px solid #E5E7EB",display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
...
function ListView() {
  ...
  return (
    <div>
      ...
      <div className="card" style={{overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 110px 100px 90px 48px",gap:8,padding:"9px 18px",borderBottom:"1px solid #F3F4F6"}}>
        ...
        {filtered.length>0 ? filtered.map(p=>{
          ...
          return (
            <div key={p.id} style={{borderBottom:"1px solid #F9FAFB"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 110px 100px 90px 48px",gap:8,padding:"11px 18px",alignItems:"center",cursor:"pointer",transition:"background 0.1s"}}
    ...
    <div style={{maxWidth:760}}>
      ...
      <div className="card" style={{padding:20,marginBottom:14}}>
        ...
        <textarea value={text} onChange={e=>setText(e.target.value)}
          placeholder="What's the hook? What angle? Dump it here before it disappears…"
    ...
    <div style={{maxWidth:900}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"320px 1fr",gap:14,alignItems:"start"}}>
        <div className="card" style={{padding:20}}>
          ...
          <textarea value={text} onChange={e=>setText(e.target.value)}
            placeholder="Hook, CTA, caption framework, hashtag group…"
              ...
              <div key={item.id} className="card card-hover" style={{padding:16,position:"relative"}}>
                ...
                {confirm===item.id ? (
                  ...
                  <button onClick={()=>setConfirm(item.id)} className="btn-ghost"
                    style={{position:"absolute",top:10,right:10,padding:5,color:"#D1D5DB"}}
...
function QuickAddBar() {
  ...
  const inp = { background:"#fff", border:"1px solid #E5E7EB", borderRadius:8, padding:"7px 12px", fontSize:13, color:"#111827", fontFamily:"'Inter',sans-serif", outline:"none", transition:"border-color 0.15s" };
  return (
...
function EditPanel() {
  ...
  const inp = { width:"100%", background:"#fff", border:"1.5px solid #E5E7EB", borderRadius:9, padding:"9px 12px", fontSize:13, color:"#111827", fontFamily:"'Inter',sans-serif", outline:"none", transition:"border-color .15s, box-shadow .15s" };
  ...
  return (
    <>
      ...
      <div className="slide-in" style={{position:"fixed",right:0,top:0,zIndex:50,height:"100%",width:"100%",maxWidth:500,display:"flex",flexDirection:"column",background:"#fff",boxShadow:"-4px 0 40px rgba(0,0,0,.1)",borderLeft:"1px solid #E5E7EB"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 18px"}}>
          ...
          <input value={form.title} onChange={sf("title")} placeholder="Post title"
            style={{...inp,fontSize:19,fontWeight:700,marginBottom:20,letterSpacing:"-0.2px"}} {...foc}/>
              ...
              {STATUSES.map(s=>{
                ...
                return (
                  <button key={s} onClick={()=>handleStatus(s)}
                    style={{display:"flex",alignItems:"center",gap:5,padding:"5px 14px",borderRadius:20,border:"1.5px solid",borderColor:active?"#6366F1":"#E5E7EB",background:active?"#EEF2FF":"#fff",color:active?"#6366F1":"#6B7280",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all 0.12s",boxShadow:active?"0 0 0 3px rgba(99,102,241,.1)":"none"}}>
...
function CommandPalette() {
  ...
  return (
    <div className="cmd-wrap fade-in" onClick={()=>setCmdOpen(false)}>
      <div className="scale-in card" style={{width:"100%",maxWidth:520,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.15)"}} onClick={e=>e.stopPropagation()}>
...
function LoadingState() {
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"60vh",gap:14}}>
      <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,boxShadow:"0 4px 16px rgba(99,102,241,.3)"}}>🌿</div>
