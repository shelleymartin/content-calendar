...
function PublishTargets({ post }){
  const{db,setErr,fetchAll}=useApp();
  const records = (db.publishing_records||[]).filter(
    r=>String(r.post_id)===String(post.id)
  );
  const connectedAccounts = db.connected_accounts||[];
  const[publishing,setPublishing]=useState(null); // platformId being published

  async function handlePublishClick(platformId){
    if(publishing) return; // prevent double-click
    const platform = SOCIAL_PLATFORMS[platformId];
    const account  = connectedAccounts.find(a=>a.platform===platformId&&a.is_connected);

    if(!account){
      setErr("Connect your "+platform.label+" account first in the Publishing tab.");
      return;
    }

    // Check post has required fields
    if(!post.caption?.trim()){
      setErr("Add a caption to this post before publishing.");
      return;
    }

    if(platformId==="instagram" && !post.video_link?.trim()){
      setErr("Instagram requires an image or video URL. Add one in the Attached Media field.");
      return;
    }
    ...
    try{
      const res = await fetch(
        "https://ilezaspkmjyxwxfacebw.supabase.co/functions/v1/publish-instagram",
        {
          method: "POST",
      ...
      if(data.error){
        setErr("Publishing failed: "+data.error);
      } else {
        // Refresh to show new publishing record
        await fetchAll(false);
      }
    }catch(e){
      setErr("Could not reach publishing server. Check your connection.");
    }finally{
      setPublishing(null);
    }
  }
          ...
          return(
            <div key={platform.id} style={{display:"flex",alignItems:"center",gap:10,
              ...
              {record ? (
                ...
                <button
                  onClick={()=>handlePublishClick(platform.id)}
                  disabled={!!publishing}
                  style={{display:"flex",alignItems:"center",gap:4,
                    padding:"4px 10px",borderRadius:6,
                    border:"1px solid",borderColor:platform.border,
                    background: publishing===platform.id ? "#F3F4F6" : platform.bg,
                    color: publishing===platform.id ? "#9CA3AF" : platform.textColor,
                    fontSize:11,fontWeight:600,
                    cursor: publishing ? "not-allowed" : "pointer",
                    fontFamily:"Inter,sans-serif",transition:"all .12s",whiteSpace:"nowrap"}}>
                  {publishing===platform.id
                    ? <><Loader2 style={{width:9,height:9}} className="spin"/>Publishing...</>
